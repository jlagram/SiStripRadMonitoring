...
void GetNumberOfTracks(TFile* f, int& ntot, int& overflow, float& mean, float& fbelow10)
{
  ...
  if(!h) { cout<<"Error : no histo '"<<histoname<<"'"<<endl; ntot=0; overflow=0; mean=0; fbelow10=0; return;}
  mean = h->GetMean();
...
TProfile* GetNHitsPerLayer(TFile* f, float Vstep=-1, string SubDet="TIB")
{
  ...
  for(unsigned int j=0; j<nentries; j++)
  {
    ...
    T->GetEntry(j);
    if(SubDet=="TOB" || SubDet=="TEC") detid/=10; // For TOB, 2 sensors
    subdet = (detid>>25)&0x7;
    if(subdet==3 || subdet==5) layer = ((detid>>14) & 0x7);
    if(subdet==4) layer = ((detid>>9) & 0x3);
    if(subdet==6) layer = ((detid>>5) & 0x7);
...
void GetTreeStat(int run=258443, string suffix="")
{
  string dirname ="/afs/cern.ch/user/p/pvaucell/CMSSW_13_0_0/src/SiStripRadMonitoring/SignalBiasScan/Analysis/SignalAnalysis/Code";
  // string dirname = "~/work/public/SiStripRadMonitoring/SignalCurves/Code";
  //string dirname= "./SiStripRadMonitoring/SignalBiasScan/Analysis/SignalAnalysis";
  //string dirname = "/afs/cern.ch/user/n/ntonon/public/tracker_aging/CMSSW_8_0_20_patch1/src/SiStripRadMonitoring/SignalBiasScan/Analysis/SignalAnalysis/Code/Outputs/";
  //string dirname= "./";
  string filename = "output_DecoMode";
  char file[500];
  sprintf(file, "%s/TIB_%s_%i%s.root", dirname.c_str(), filename.c_str(), run, suffix.c_str()); 
  ...
  // Nhits in TOB
  char fileTOB[500];
  sprintf(fileTOB, "%s/TOB_%s_%i%s.root", dirname.c_str(), filename.c_str(), run, suffix.c_str());
  ...
  cout<<"    L6 : "<<pfTOB->GetBinContent(7)<<", rms: "<<pfTOB->GetBinError(7)<<endl<<endl;


  // Nhits in TID
  char fileTID[500];
  sprintf(fileTID, "%s/TID_%s_%i%s.root", dirname.c_str(), filename.c_str(), run, suffix.c_str());
  TFile* fTID = TFile::Open(fileTID);
  TProfile *pfTID = GetNHitsPerLayer(fTID, 300, "TID");
  cout<<"  #hits/module in TID"<<endl;
  cout<<"    R1 : "<<pfTID->GetBinContent(2)<<", rms: "<<pfTID->GetBinError(2)<<endl;
  cout<<"    R2 : "<<pfTID->GetBinContent(3)<<", rms: "<<pfTID->GetBinError(3)<<endl;
  cout<<"    R3 : "<<pfTID->GetBinContent(4)<<", rms: "<<pfTID->GetBinError(4)<<endl<<endl;


  // Nhits in TEC
  char fileTEC[500];
  sprintf(fileTEC, "%s/TEC_%s_%i%s.root", dirname.c_str(), filename.c_str(), run, suffix.c_str());
  TFile* fTEC = TFile::Open(fileTEC);
  TProfile *pfTEC = GetNHitsPerLayer(fTEC, 300, "TEC");
  cout<<"  #hits/module in TEC at 300V (wheel -7 in small scans)"<<endl;
  cout<<"    R1 : "<<pfTEC->GetBinContent(2)<<", rms: "<<pfTEC->GetBinError(2)<<endl;
  cout<<"    R2 : "<<pfTEC->GetBinContent(3)<<", rms: "<<pfTEC->GetBinError(3)<<endl;
  cout<<"    R3 : "<<pfTEC->GetBinContent(4)<<", rms: "<<pfTEC->GetBinError(4)<<endl;
  cout<<"    R4 : "<<pfTEC->GetBinContent(5)<<", rms: "<<pfTEC->GetBinError(5)<<endl;
  cout<<"    R5 : "<<pfTEC->GetBinContent(6)<<", rms: "<<pfTEC->GetBinError(6)<<endl;
  cout<<"    R6 : "<<pfTEC->GetBinContent(7)<<", rms: "<<pfTEC->GetBinError(7)<<endl;
  cout<<"    R7 : "<<pfTEC->GetBinContent(8)<<", rms: "<<pfTEC->GetBinError(8)<<endl<<endl;
