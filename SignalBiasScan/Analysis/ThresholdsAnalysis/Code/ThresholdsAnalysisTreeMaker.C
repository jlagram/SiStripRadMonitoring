#define ThresholdsAnalysisTreeMaker_cxx

#include "ThresholdsAnalysisTreeMaker.h"
...
// Main method //
//-------------//
void ThresholdsAnalysisTreeMaker::Loop()
{
...
void ThresholdsAnalysisTreeMaker::PrintCluster(TreeFullHit *hit)
{
  ...
  cout<<"nStrips "<<nStrips<<" charge "<<hit->chargeAngleCorr<<" noise "<<hit->noise<<endl;
  cout<<" seed charge "<<hit->seedChargeAngleCorr<<endl;
  cout<<"S : ";
  ...
  cout<<"SoN : ";
  for(unsigned int istr=0; istr<nStrips; istr++)
    cout<< hit->stripCharges[istr]/hit->stripNoises[istr]*hit->stripGains[istr] <<" ";
  cout<<endl;
  
  cout<<" seed index "<<seedIndex<<" "<<max_element(hit->stripCharges.begin(), hit->stripCharges.end()) - hit->stripCharges.begin()<<endl;
  cout<<" cluster noise "<<clusterNoise<<endl;
}

void ThresholdsAnalysisTreeMaker::Clusterize(TreeFullHit *hit)
{
  float signalEff = 0.98;
  float seedThresh = 3;
  float stripThresh = 2;
  float clusThresh = 5;
  unsigned int maxSequentialHoles = 0;
  
  /*unsigned int seedIndex = max_element(hit->stripCharges.begin(), hit->stripCharges.end()) - hit->stripCharges.begin();
  
  float seedSoN = hit->stripCharges[seedIndex]*hit->stripGains[seedIndex]/hit->stripNoises[seedIndex];
  if(seedSoN<6)
  {
    PrintCluster( hit );
    cout<<" seed SoN "<<seedSoN<<endl;
  }*/
  ...
  vector<float> clus_S;
  vector<unsigned int> clus_nstr;
  vector<float> clus_first_ADC;
  vector<float> clus_last_ADC;
  ...
  for(unsigned int istr=0; istr<nStrips; istr++)
  {
       ...
       //(holes > MaxSequentialBad || !allBadBetween(firstStrip, itstr)))
	{
	  ...
	  {
		/*cout<<"new cluster with "<<ADCs.size()<<" strips, charge : "<<signal<<endl;
		cout<<"S : ";
		for(unsigned int is=0; is<ADCs.size(); is++)
    	  cout<<ADCs[is]<<" ";
		cout<<endl;*/
		clus_S.push_back(signal);
		clus_nstr.push_back(ADCs.size());
		clus_first_ADC.push_back(ADCs[0]);
		clus_last_ADC.push_back(ADCs.back());
		
		ADCs.clear();
		candidateLacksSeed = true;
		noiseSquared = 0;
		lastStrip = istr;
		signal = 0;
	  }
	...
	if(!ADCs.empty())
	for(unsigned int ihol=lastStrip+1; ihol<istr; ihol++) ADCs.push_back(0); // holes

	ADCs.push_back( hit->stripCharges[istr] );
  ...
  if( !candidateLacksSeed && signal > sqrt(noiseSquared)*clusThresh )
  {
    /*cout<<"initial cluster size : "<<nStrips<<", charge : "<<hit->chargeAngleCorr/fabs(cos(hit->angleForCorr))<<endl;
	PrintCluster(hit);
	cout<<"end cluster with "<<ADCs.size()<<" strips, charge : "<<signal<<endl;
	cout<<"S : ";
	for(unsigned int is=0; is<ADCs.size(); is++)
      cout<<ADCs[is]<<" ";
	cout<<endl;*/
  ...
  if(clus_S.size()!=1 || (clus_S.size()==1 && clus_nstr[0]!=nStrips))
  {
    cout<<"initial cluster size : "<<nStrips<<", charge : "<<hit->chargeAngleCorr/fabs(cos(hit->angleForCorr))<<endl;
    PrintCluster(hit);
	if(clus_S.size()==0)  cout<<"no re-clusterized cluster"<<endl;
	for(unsigned int icl=0; icl<clus_S.size(); icl++)
	{
	  cout<<"new cluster with "<<clus_nstr[icl]<<" strips, charge : "<<clus_S[icl]<<endl;
	  cout<<" first strip : "<<clus_first_ADC[icl]<<" last strip : "<<clus_last_ADC[icl]<<endl;
	}
	cout<<endl;
  }
...
void ThresholdsAnalysisTreeMaker::FillHitInfo(std::map<ULong64_t , std::vector<TH1F*> > &HistSoN, std::map<ULong64_t , std::vector<TProfile*> > &ProfVsAngle,
...
void ThresholdsAnalysisTreeMaker::FillHistos(std::map<ULong64_t , std::vector<TH1F*> > &HistSoN, std::map<ULong64_t , std::vector<TProfile*> > &ProfVsAngle,
...
void ThresholdsAnalysisTreeMaker::FitHistos(std::map<ULong64_t , std::vector<TH1F*> > &HistSoN, string output_file, 
...
void ThresholdsAnalysisTreeMaker::FitProfiles(std::map<ULong64_t , std::vector<TProfile*> > &ProfVsAngle, string output_file){
