...
void DrawOneModule(string dirname, string subdet, string antype, string ref, const int NF, vector<string> runs, vector<float> lumis, ULong64_t detid, bool useflu=false, bool print=false, bool use_curvature=true, bool superimpose_simu=false, bool draw_vdep_lab=true, bool draw_fit=false, bool draw_gray_band=false)
{
  ...
  string filename = "DECO_"+antype+"_"+subdet;//allModules_ when full scan !!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  if(superimpose_simu)
  {
	...
	if(Check_File_Existence(simufile_name) )
	{
        ...
        else
        {
			...
			else if (choice_simu == 5 && subdet == "TOB"){g_simu_name = "lumigr_TOB_L1";}
			else if (choice_simu == 5 && subdet == "TEC"){g_simu_name = "lumigr_TEC_R5";}
...
// -- using https://twiki.cern.ch/twiki/pub/CMS/Internal/FigGuidelines
	...
	if(subdet=="TID") detid_text = "Detid " + Convert_Number_To_TString(detid) + " - "+subdet+" R"+Convert_Number_To_TString((ULong64_t) GetLayer(detid));
	latex.SetTextFont(42);
...
//    DrawOneModule(dirname, "TIB", antype, ref, NF, runs, lumis, 369125870, draw_vdep_lab, draw_fit);
	...
	//Ivan Request may 2023
   	DrawOneModule(dirname, "TIB", antype, ref, NF, runs, lumis, 369124397, draw_vdep_lab, draw_fit);
	DrawOneModule(dirname, "TIB", antype, ref, NF, runs, lumis, 369124414, draw_vdep_lab, draw_fit);
	DrawOneModule(dirname, "TIB", antype, ref, NF, runs, lumis, 369124422, draw_vdep_lab, draw_fit);
	DrawOneModule(dirname, "TIB", antype, ref, NF, runs, lumis, 369124662, draw_vdep_lab, draw_fit);
	DrawOneModule(dirname, "TIB", antype, ref, NF, runs, lumis, 369124666, draw_vdep_lab, draw_fit);
	DrawOneModule(dirname, "TIB", antype, ref, NF, runs, lumis, 369124694, draw_vdep_lab, draw_fit);
	DrawOneModule(dirname, "TIB", antype, ref, NF, runs, lumis, 369124710, draw_vdep_lab, draw_fit);
	DrawOneModule(dirname, "TIB", antype, ref, NF, runs, lumis, 369124726, draw_vdep_lab, draw_fit);
	DrawOneModule(dirname, "TIB", antype, ref, NF, runs, lumis, 369124774, draw_vdep_lab, draw_fit);
	DrawOneModule(dirname, "TIB", antype, ref, NF, runs, lumis, 369125870, draw_vdep_lab, draw_fit);
 }
 if(subdet=="TOB")
 {
...
const int ProduceDetidLayerFile( string subdet,  int layer=0 )
	{
		TString name = "";
		TString outname = "";
		TString Layer = "";
		...
		if (subdet == "TIB")
			{
				name = "/afs/cern.ch/user/p/pvaucell/CMSSW_14_0_14/src/SiStripRadMonitoring/SignalBiasScan/Analysis/CurvesAnalysis/detid_lists/TIB_list.txt";
				outname = "TIB_L"+Layer+"_list.txt";
			}
		else if (subdet == "TOB")
			{
				name = "/afs/cern.ch/user/p/pvaucell/CMSSW_14_0_14/src/SiStripRadMonitoring/SignalBiasScan/Analysis/CurvesAnalysis/detid_lists/TOB_list.txt";
				outname = "TOB_L"+Layer+"_list.txt";
			}
		else if (subdet == "TEC")
			{
				name = "/afs/cern.ch/user/p/pvaucell/CMSSW_14_0_14/src/SiStripRadMonitoring/SignalBiasScan/Analysis/CurvesAnalysis/detid_lists/TEC_list.txt";
				outname = "TEC_R"+Layer+"_list.txt";
			}
		...
		int ID;
		int count = 0;
		std::cout<<" Start of File "<<outname<<std::endl;
		while (std::getline(input, line) )
			{
				...
				if(subdet=="TIB")  // TIB : subdetid==3
					{
						if(((ID>>14)&0x7)!= (layer+1)) continue; //return 1;
						// std::cout<<"count : "<<count<<" ID "<<ID<<" vinit "<<vinit<<std::endl;
						output<<ID<<" "<<vinit<<endl;
						count+=1;
					}  
				else if(subdet=="TOB") // TOB subdetid==5
					{       
						if(((ID>>14)&0x7)!= (layer+1)) continue; //return 5;
						output<<ID<<" "<<vinit<<endl;
						count+=1;
					}   
				else if (subdet == "TEC") // Martin Delcourt request : TEC R5 wheel 9 
					{
						if (  !(((ID>>25)&0x7) == 6   && ((ID >> 5) & 7) == (layer+1))   ) continue; // if you want wheel selection =>  &&  ((ID>>14)&0xF) == 9
						output<<ID<<" "<<vinit<<endl;
						count+=1;
					}
			}
		input.close();
		output.close();
		std::cout<<" End of File "<<name<<std::endl;
...
TH1F* DrawHistoModules_FullScan(string dirname, string subdet, string antype, string ref, string run, bool show=true, bool use_curvature=true, int layer=0)
{
  if(subdet!="TIB" && subdet!="TOB" && subdet!="TEC") {cout<<__LINE__<<" : Subdet '"<<subdet<<"' not allowed."<<endl; return 0;}
	...
	const int Ndetid = ProduceDetidLayerFile(subdet, layer);// Produce the list of detids for the layer

	const int Ndet_TIB = Ndetid;
	ULong64_t Detids_TIB[Ndet_TIB]={0};
	double vfd_ref_TIB[Ndet_TIB]= {0};

	const int Ndet_TOB = Ndetid;
	ULong64_t Detids_TOB[Ndet_TOB]={0};
	double vfd_ref_TOB[Ndet_TOB]= {0};
	
	const int Ndet_TEC = Ndetid;
	ULong64_t Detids_TEC[Ndet_TEC]={0};
	double vfd_ref_TEC[Ndet_TEC]= {0};

	TString Layer = std::to_string(layer+1);
		//--------------Full Scans -----------------//

	if (subdet == "TIB")
		{
			std::cout<<"Strat to analyze TIB"<<std::endl;
			std::ifstream input( "/afs/cern.ch/user/p/pvaucell/CMSSW_14_0_14/src/SiStripRadMonitoring/SignalBiasScan/Analysis/CurvesAnalysis/TIB_L"+Layer+"_list.txt" );//TIB_list
			while (std::getline(input, line) )
				{
					std::istringstream iss(line);
					if (!(iss >> ID >> vinit)) { break; } // error
					...
					{
						Detids_TIB[cnt]=ID;
						vfd_ref_TIB[cnt]= vinit;
						cnt+=1;
						// std::cout<<"count : "<<cnt<<" ID "<<ID<<" vinit "<<vinit<<std::endl;
					}
				}
			input.close();
		}
		...
		{
			std::ifstream input( "/afs/cern.ch/user/p/pvaucell/CMSSW_14_0_14/src/SiStripRadMonitoring/SignalBiasScan/Analysis/CurvesAnalysis/detid_lists/TOB_L"+Layer+"_list.txt" );
			while (std::getline(input, line) )
				{
					std::istringstream iss(line);
					if (!(iss >> ID >> vinit)) { break; } // error
		...
		{
			std::ifstream input( "/afs/cern.ch/user/p/pvaucell/CMSSW_14_0_14/src/SiStripRadMonitoring/SignalBiasScan/Analysis/CurvesAnalysis/detid_lists/TEC_R"+Layer+"_list.txt" );
			while (std::getline(input, line) )
				{
					std::istringstream iss(line);
					if (!(iss >> ID >> vinit)) { break; } // error
...
TH1F* DrawHistoDiffModules_FullScan(string dirname, string subdet, string antype, string ref, string run, bool show=true, bool use_curvature=true, int layer=0)
{
  if(subdet!="TIB" && subdet!="TOB" && subdet!="TEC") {cout<<__LINE__<<" : Subdet '"<<subdet<<"' not allowed."<<endl; return 0;}
	...
	if (subdet == "TIB")
		...
		{
			std::ifstream input( "/afs/cern.ch/user/p/pvaucell/CMSSW_14_0_14/src/SiStripRadMonitoring/SignalBiasScan/Analysis/CurvesAnalysis/TOB_L"+Layer+"_list.txt" );
			while (std::getline(input, line) )
				{
					std::istringstream iss(line);
					if (!(iss >> ID >> vinit)) { break; } // error
		...
		{
			std::ifstream input( "/afs/cern.ch/user/p/pvaucell/CMSSW_14_0_14/src/SiStripRadMonitoring/SignalBiasScan/Analysis/CurvesAnalysis/TEC_R"+Layer+"_list.txt" );
			while (std::getline(input, line) )
				{
					std::istringstream iss(line);
					if (!(iss >> ID >> vinit)) { break; } // error
...
TGraphErrors* DrawDiffModules_FullScan(string dirname, string subdet, string antype, string ref, const int NF, vector<string> runs, vector<float> lumis, bool useflu=false, bool use_curvature=true, int layer=0, bool draw_fit=true)
{
  ...
  TH1F* href = DrawHistoDiffModules_FullScan(dirname, subdet, antype, ref, "labref", useflu, use_curvature, layer); //Last arg : TEC layer we consider
  if(!href) {cout<<FRED("hdiff is null! Abort")<<endl; return 0;}
	...
	for(int i=0; i<NF; i++)
  {
    TH1F* hdiff = DrawHistoDiffModules_FullScan(dirname, subdet, antype, ref, runs[i], useflu, use_curvature, layer);
	...
	cout<<"--- Run "<<runs[i]<<endl;
	cout<<"hdiff : "<<hdiff->GetEntries()<<" entries / Mean = "<<hdiff->GetMean()<<" / RMS : "<<hdiff->GetRMS()<< " / Mean Error : "<<hdiff->GetMeanError() <<endl;
	cout<<"href : "<<href->GetEntries()<<" entries / Mean = "<<href->GetMean()<<" / RMS : "<<href->GetRMS()<<" / Mean Error : "<<hdiff->GetMeanError() <<endl;

	g->SetPoint(ipt, lumi, hdiff->GetMean());//-href->GetMean());

	if(useRmsAsErrors) g->SetPointError(ipt, 0, hdiff->GetMeanError());
    else g->SetPointError(ipt, 0, hdiff->GetMeanError());

	if(i==0)
	{
        if(useRmsAsErrors) g->SetPointError(ipt, 0, href->GetMeanError());
...
if(writeExtraText) latex.DrawLatex(c1->GetLeftMargin() + 0.1, 0.953, extraText);
  ...
  if(subdet == "TEC")
  {
	...
	TString nameroot = name+"_fullscan.root";
  name+="_fullscan.pdf";
  //name+=".pdf";
  c1->SaveAs(nameroot);
...
TGraphErrors* DrawVfdModules_FullScan(string dirname, string subdet, string antype, string ref, const int NF, vector<string> runs, vector<float> lumis, bool useflu=false, bool use_curvature=true, int layer=0, bool draw_fit=true)
{
  bool useRmsAsErrors=true;
  cout<<"Use RMS error : "<<useRmsAsErrors<<" // Use Mean Error : "<<1-useRmsAsErrors<<endl;
  ...
  TH1F* href = DrawHistoModules_FullScan(dirname, subdet, antype, ref, "labref", useflu, use_curvature, layer); //Last arg : TEC layer we consider
  if(!href) {cout<<FRED("hdiff is null! Abort")<<endl; return 0;}
	...
	for(int i=0; i<NF; i++)
  {
    TH1F* hdiff = DrawHistoModules_FullScan(dirname, subdet, antype, ref, runs[i], useflu, use_curvature, layer);
...
if(writeExtraText) latex.DrawLatex(c1->GetLeftMargin() + 0.1, 0.953, extraText);
  ...
  if(!use_curvature) {name += subdet + "_line_"+antype;}
  else {name += subdet + "_kink_"+antype;}
  if(subdet == "TEC")
  {
  	if(layer != 0) {name+= "_R"+Convert_Number_To_TString(layer);}
  	else {name+= "_allRings";}
...
TH1F* DrawHistoModules_SmallScan(string dirname, string subdet, string antype, string ref, string run, bool show=true, bool use_curvature=true, int layer=0)
{
  if(subdet!="TIB" && subdet!="TOB" && subdet!="TEC") {cout<<__LINE__<<" : Subdet '"<<subdet<<"' not allowed."<<endl; return 0;}
...
TGraphErrors* DrawVfdModules_SmallScan(string dirname, string subdet, string antype, string ref, const int NF, vector<string> runs, vector<float> lumis, bool useflu=false, bool use_curvature=true, int layer=0, bool draw_fit=true)
{
  bool useRmsAsErrors=true;
  cout<<"Use RMS error : "<<useRmsAsErrors<<" // Use Mean Error : "<<1-useRmsAsErrors<<endl;
  ...
  TH1F* href = DrawHistoModules_SmallScan(dirname, subdet, antype, ref, "labref", useflu, use_curvature, layer); //Last arg : TEC layer we consider
  if(!href) {cout<<FRED("hdiff is null! Abort")<<endl; return 0;}
	...
	for(int i=0; i<NF; i++)
  {
    TH1F* hdiff = DrawHistoModules_SmallScan(dirname, subdet, antype, ref, runs[i], useflu, use_curvature, layer);
...
void Superimpose_DrawDiffModules_SmallScan(string dirname, string subdet, string analysis, string ref, const int NF, vector<string> runs, vector<float> lumis, bool useflu, bool use_curvature, bool draw_fit=true)
{
  ...
  if(compare_methods)
  {
     name = "plots/superimpose_curves/superimpose_diffMethods_"+subdet+"_";
     name+= analysis;
  }
  else
  {
      name = "plots/superimpose_curves/superimpose_diffObservables_"+subdet+"_";
...
//Superimpose 2 "diff kink" curves (to have Signal & Clusterwidth on same plot)
void Superimpose_DrawModules_SmallScan(bool superimpose_simu, string dirname, string subdet, string analysis, string ref, const int NF, vector<string> runs, vector<float> lumis, bool useflu, bool use_curvature, bool draw_fit=true)
{
    bool compare_methods = true; //false <-> compare observables instead
    ...
    if(compare_methods) //compare methods
    {
        g_1 = DrawVfdModules_SmallScan(dirname, subdet, analysis, ref, NF, runs, lumis, useflu, 1, 0, draw_fit);
        g_2 = DrawVfdModules_SmallScan(dirname, subdet, analysis, ref, NF, runs, lumis, useflu, 0, 0, draw_fit);
    }
    else //compare observables
    {
        g_1 = DrawVfdModules_SmallScan(dirname, subdet, "Signal", ref, NF, runs, lumis, useflu, use_curvature, 0, draw_fit);
        g_2 = DrawVfdModules_SmallScan(dirname, subdet, "ClusterWidth", ref, NF, runs, lumis, useflu, use_curvature, 0, draw_fit);
    }
...
double y_tmp = 0, x_tmp=0, y_max=0, y_min=9999, flumax=0;
  ...
  if(superimpose_simu)
  {
	...
	if(Check_File_Existence(simufile_name) )
	{
        ...
        // {
            ...
            else g_simu_name = "lumigr_TIB_L1";
			// g_simu_name = "feqgr_"+ts_detid;
        // }
		...
		for(int i=0; i<g_simu->GetN(); i++)
		{
			...
			{
				// if(!draw_vs_fluence) {g_simu->SetPoint(i, GetLumiFromFluence(x_tmp, detid, subdet), y_tmp);} //In files from Cbart there is only fluence
				g_simu->SetPoint(i, x_tmp*pow(10, -12), y_tmp);
			}
			// else if(choice_simu == 3) //Non-physical decrease above 195fb
			// {
			// 	if(x_tmp > 150)
  ...
  g_1->SetMinimum(0.); //So that we can ~see where simu predicts inversion
  g_2->SetMinimum(0.); //So that we can ~see where simu predicts inversion
  TLegend* leg;
  if(compare_methods) {leg = new TLegend(.60,.80,.93,.90);}
  if(superimpose_simu && g_simu != 0)
  {
...
//---------------------------------------------------------------------//
  ...
  if(compare_methods)
  {
     name = "plots/superimpose_curves/superimpose_Methods_"+subdet+"_";
     name+= analysis;
  }
  else
  {
      name = "plots/superimpose_curves/superimpose_Observables_"+subdet+"_";
      if(use_curvature) {name+= "kink";}
      else {name+= "line";}
  }
  TString  nameroot = name+".root";
...
//Superimpose 2 "diff kink" curves (to have Signal & Clusterwidth on same plot)
void Superimpose_DrawDiffModules_FullScan(string dirname, string subdet, string analysis, string ref, const int NF, vector<string> runs, vector<float> lumis, bool useflu, bool use_curvature, bool draw_fit=true)
{
    bool compare_methods = true; //false <-> compare observables instead
    ...
    if(compare_methods) //compare methods
    {
        g_1 = DrawDiffModules_FullScan(dirname, subdet, analysis, ref, NF, runs, lumis, useflu, 1, 0, draw_fit);
        g_2 = DrawDiffModules_FullScan(dirname, subdet, analysis, ref, NF, runs, lumis, useflu, 0, 0, draw_fit);
    }
    else //compare observables
    {
        g_1 = DrawDiffModules_FullScan(dirname, subdet, "Signal", ref, NF, runs, lumis, useflu, use_curvature, 0, draw_fit);
        g_2 = DrawDiffModules_FullScan(dirname, subdet, "ClusterWidth", ref, NF, runs, lumis, useflu, use_curvature, 0, draw_fit);
    }
  ...
  if(compare_methods)
  {
     name = "plots/superimpose_curves/superimpose_diffMethods_FullScan_"+subdet+"_";
     name+= analysis;
  }
  else
  {
      name = "plots/superimpose_curves/superimpose_diffObservables_FullScan_"+subdet+"_";
      if(use_curvature) {name+= "kink";}
      else {name+= "line";}
  }
  TString  nameroot = name+".root";
...
//Superimpose 2 "diff kink" curves (to have Signal & Clusterwidth on same plot)
void Superimpose_DrawModules_FullScan(bool superimpose_simu, string dirname, string subdet, string analysis, string ref, const int NF, vector<string> runs, vector<float> lumis, bool useflu, bool use_curvature, bool draw_fit=true)
{
    bool compare_methods = true; //false <-> compare observables instead
    ...
    if(compare_methods) //compare methods
    {
        g_1 = DrawVfdModules_FullScan(dirname, subdet, analysis, ref, NF, runs, lumis, useflu, 1, 0, draw_fit);
        g_2 = DrawVfdModules_FullScan(dirname, subdet, analysis, ref, NF, runs, lumis, useflu, 0, 0, draw_fit);
    }
    else //compare observables
    {
        g_1 = DrawVfdModules_FullScan(dirname, subdet, "Signal", ref, NF, runs, lumis, useflu, use_curvature, 0, draw_fit);
        g_2 = DrawVfdModules_FullScan(dirname, subdet, "ClusterWidth", ref, NF, runs, lumis, useflu, use_curvature, 0, draw_fit);
    }
...
double y_tmp = 0, x_tmp=0, y_max=0, y_min=9999, flumax=0;
  ...
  if(superimpose_simu)
  {
	...
	if(Check_File_Existence(simufile_name) )
	{
        ...
        // {
			if (choice_simu == 5 && subdet == "TIB") {g_simu_name = "lumigr_TIB_L1";}
			if (choice_simu == 5 && subdet == "TOB") {g_simu_name = "lumigr_TOB_L1";}
			if (choice_simu == 5 && subdet == "TEC") {g_simu_name = "lumigr_TEC_R1";}
            else g_simu_name = "lumigr_TIB_L1";
			// g_simu_name = "feqgr_"+ts_detid;
        // }
...
//---------------------------------------------------------------------//
  ...
  if(compare_methods)
  {
     name = "plots/superimpose_curves/superimpose_Methods_FullScan_"+subdet+"_";
     name+= analysis;
  }
  else
  {
      name = "plots/superimpose_curves/superimpose_Observables_FullScan_"+subdet+"_";
      if(use_curvature) {name+= "kink";}
      else {name+= "line";}
  }
  TString  nameroot = name+".root";
...
//--- Modules
  ...
  if(subdet=="TIB")
  {
	  ...
	  //TIB+ L1  until run 203836 (included) 
	...
	//   DrawOneModule(dirname, "TIB", type, "", NF, runs, lumis, 369124397, usefluence, true, use_curvature, superimpose_simu, draw_vdep_lab, draw_fit, draw_gray_band);
	//   DrawOneModule(dirname, "TIB", type, "", NF, runs, lumis, 369124414, usefluence, true, use_curvature, superimpose_simu, draw_vdep_lab, draw_fit, draw_gray_band);
	//   DrawOneModule(dirname, "TIB", type, "", NF, runs, lumis, 369124422, usefluence, true, use_curvature, superimpose_simu, draw_vdep_lab, draw_fit, draw_gray_band);
	//   DrawOneModule(dirname, "TIB", type, "", NF, runs, lumis, 369124662, usefluence, true, use_curvature, superimpose_simu, draw_vdep_lab, draw_fit, draw_gray_band);
	//   DrawOneModule(dirname, "TIB", type, "", NF, runs, lumis, 369124666, usefluence, true, use_curvature, superimpose_simu, draw_vdep_lab, draw_fit, draw_gray_band);
	//   DrawOneModule(dirname, "TIB", type, "", NF, runs, lumis, 369124694, usefluence, true, use_curvature, superimpose_simu, draw_vdep_lab, draw_fit, draw_gray_band);
	//   DrawOneModule(dirname, "TIB", type, "", NF, runs, lumis, 369124710, usefluence, true, use_curvature, superimpose_simu, draw_vdep_lab, draw_fit, draw_gray_band);
	//   DrawOneModule(dirname, "TIB", type, "", NF, runs, lumis, 369124726, usefluence, true, use_curvature, superimpose_simu, draw_vdep_lab, draw_fit, draw_gray_band);
	//   DrawOneModule(dirname, "TIB", type, "", NF, runs, lumis, 369124774, usefluence, true, use_curvature, superimpose_simu, draw_vdep_lab, draw_fit, draw_gray_band);
 	//   DrawOneModule(dirname, "TIB", type, "", NF, runs, lumis, 369125870, usefluence, true, use_curvature, superimpose_simu, draw_vdep_lab, draw_fit, draw_gray_band);
  ...
  else if(subdet=="TID")
  {
	...
	// TIDminus_4_2_1_1///channel002
	  DrawOneModule(dirname, "TID", type, "", NF, runs, lumis, 402666258, usefluence, true, use_curvature, superimpose_simu, draw_vdep_lab, draw_fit, draw_gray_band);
	  DrawOneModule(dirname, "TID", type, "", NF, runs, lumis, 402666262, usefluence, true, use_curvature, superimpose_simu, draw_vdep_lab, draw_fit, draw_gray_band);
	  DrawOneModule(dirname, "TID", type, "", NF, runs, lumis, 402666266, usefluence, true, use_curvature, superimpose_simu, draw_vdep_lab, draw_fit, draw_gray_band);
		// using the CW/kink method
									//87/82/0
	// TIDminus_4_2_1_1///channel003

	  DrawOneModule(dirname, "TID", type, "", NF, runs, lumis, 402666257, usefluence, true, use_curvature, superimpose_simu, draw_vdep_lab, draw_fit, draw_gray_band);
	  DrawOneModule(dirname, "TID", type, "", NF, runs, lumis, 402666261, usefluence, true, use_curvature, superimpose_simu, draw_vdep_lab, draw_fit, draw_gray_band);
	  DrawOneModule(dirname, "TID", type, "", NF, runs, lumis, 402666265, usefluence, true, use_curvature, superimpose_simu, draw_vdep_lab, draw_fit, draw_gray_band);
									// 92/92/0
	// TIDminus_4_2_1_2//channel003

	  DrawOneModule(dirname, "TID", type, "", NF, runs, lumis, 402666269, usefluence, true, use_curvature, superimpose_simu, draw_vdep_lab, draw_fit, draw_gray_band);
	  DrawOneModule(dirname, "TID", type, "", NF, runs, lumis, 402666273, usefluence, true, use_curvature, superimpose_simu, draw_vdep_lab, draw_fit, draw_gray_band);
	  DrawOneModule(dirname, "TID", type, "", NF, runs, lumis, 402666277, usefluence, true, use_curvature, superimpose_simu, draw_vdep_lab, draw_fit, draw_gray_band);
									// 0/ 81/92V
	// TIDminus_4_2_1_2//channel002
	  DrawOneModule(dirname, "TID", type, "", NF, runs, lumis, 402666270, usefluence, true, use_curvature, superimpose_simu, draw_vdep_lab, draw_fit, draw_gray_band);
	  DrawOneModule(dirname, "TID", type, "", NF, runs, lumis, 402666274, usefluence, true, use_curvature, superimpose_simu, draw_vdep_lab, draw_fit, draw_gray_band);
	  DrawOneModule(dirname, "TID", type, "", NF, runs, lumis, 402666278, usefluence, true, use_curvature, superimpose_simu, draw_vdep_lab, draw_fit, draw_gray_band);
									// 0/ 75 /75V
	// TIDminus_4_2_1_4//channel003
	  DrawOneModule(dirname, "TID", type, "", NF, runs, lumis, 402666137, usefluence, true, use_curvature, superimpose_simu, draw_vdep_lab, draw_fit, draw_gray_band);
	  DrawOneModule(dirname, "TID", type, "", NF, runs, lumis, 402666141, usefluence, true, use_curvature, superimpose_simu, draw_vdep_lab, draw_fit, draw_gray_band);
	  DrawOneModule(dirname, "TID", type, "", NF, runs, lumis, 402666145, usefluence, true, use_curvature, superimpose_simu, draw_vdep_lab, draw_fit, draw_gray_band);
									// 0/0/90V
	// TIDminus_4_2_1_4//channel002
	  DrawOneModule(dirname, "TID", type, "", NF, runs, lumis, 402666138, usefluence, true, use_curvature, superimpose_simu, draw_vdep_lab, draw_fit, draw_gray_band);
	  DrawOneModule(dirname, "TID", type, "", NF, runs, lumis, 402666142, usefluence, true, use_curvature, superimpose_simu, draw_vdep_lab, draw_fit, draw_gray_band);
	  DrawOneModule(dirname, "TID", type, "", NF, runs, lumis, 402666146, usefluence, true, use_curvature, superimpose_simu, draw_vdep_lab, draw_fit, draw_gray_band);
										// 0/86/80V
	// TIDminus_4_2_2_1//channel003
	  DrawOneModule(dirname, "TID", type, "", NF, runs, lumis, 402666777, usefluence, true, use_curvature, superimpose_simu, draw_vdep_lab, draw_fit, draw_gray_band);
	  DrawOneModule(dirname, "TID", type, "", NF, runs, lumis, 402666781, usefluence, true, use_curvature, superimpose_simu, draw_vdep_lab, draw_fit, draw_gray_band);
	  DrawOneModule(dirname, "TID", type, "", NF, runs, lumis, 402666785, usefluence, true, use_curvature, superimpose_simu, draw_vdep_lab, draw_fit, draw_gray_band);
										// 0/90/97V
	// TIDminus_4_2_2_1//channel002
	  DrawOneModule(dirname, "TID", type, "", NF, runs, lumis, 402666778, usefluence, true, use_curvature, superimpose_simu, draw_vdep_lab, draw_fit, draw_gray_band);
	  DrawOneModule(dirname, "TID", type, "", NF, runs, lumis, 402666782, usefluence, true, use_curvature, superimpose_simu, draw_vdep_lab, draw_fit, draw_gray_band);
	  DrawOneModule(dirname, "TID", type, "", NF, runs, lumis, 402666786, usefluence, true, use_curvature, superimpose_simu, draw_vdep_lab, draw_fit, draw_gray_band);
										//90/210/90
  }
...
pair<vector<double>, vector<double> > Compute_Mean_Vfd_Drop_Per_Layer(TString dirname, TString subdet, TString antype, TString run, double lumi, bool return_fluence)
{
	...
	TString inputfile_name = dirname+"/DECO_allModules_"+antype+"_"+subdet+"_line_"+run+".root";
	std::cout<<"inputfile_name = "<<inputfile_name<<std::endl;
    ...
    for(UInt_t ie = 0; ie < nentries; ie++)
    {
		...
		for(int idet=0; idet<v_smallDetids.size(); idet++)
		{
			if(odetid == v_smallDetids[idet])
			{
				...
				// if(debug) {cout<<"mean_vfd_smallModules = "<<mean_vfd_smallModules<<endl;}
			}
		}
		// std::cout<<"layer : "<<olayer<<std::endl;// Il y a bien des modules dans chaque layer
		// cout<<"vfd_drop = "<<vfd_drop<<endl;// les valeurs sont correctes

		mean_vfd_allModules+= vfd_drop;//isue starts here
...
void Plot_Mean_Vfd_Drop_Per_Layer(TString dirname, TString antype, TString run = "", double lumi = 0)
{
	...
	// v_runs.push_back("368669"); lumis.push_back(58.16+194.68); 
	// v_runs.push_back("373060"); lumis.push_back(72.69+194.68);
	// v_runs.push_back("375658"); lumis.push_back(73.86+194.68);//HI

	// v_runs.push_back("382655");  lumis.push_back(194.68+111.53); //small
	// v_runs.push_back("385515");  lumis.push_back(194.68+172.75);//small
	// v_runs.push_back("386863"); lumis.push_back(194.68+194.48); //small
		...
		for(int idet=0; idet<v_subdet.size(); idet++)
		{
			...
			for(int ibin=0; ibin<v_mean_vfd_fluence.first.size(); ibin++)
			{
				...
				else //1 histo per subdet
				{
					...
					if(idet == 1 || (idet == 3 && ibin > 3) ) {h_color->SetBinContent(ibin_tmp + ibin + 1, v_mean_vfd_fluence.first[ibin]);std::cout<<" Coloring thick sensors"<<std::endl;}
				}
				// cout<<"subdet"<<v_subdet[idet]<<" + SetBinContent bin "<<ibin_tmp + ibin + 1<<", v_mean_vfd_fluence.first[ibin] = "<<v_mean_vfd_fluence.first[ibin]<<endl;
			}
	...
	for(int iter=0; iter<nof_iter; iter++)
	{
		...
		if (iter == 0){v_h[iter]->Draw("HIST");}
		else{v_h[iter]->Draw("HIST SAME");}
	}
	...
	if(v_runs.size() == 1)
	{
		...
		h_color->Draw("HISTSAME");
		std::cout<<"Drawing colored bins"<<std::endl;
	}
...
void Plot_Mean_Vfd_Drop_Per_Layer_MultipleScans(TString dirname, TString antype="ClusterWidth")
{
	...
	// v_runs.push_back("365843");lumis.push_back(41.42+194.68);// !!Missing // //-- FULL
		//2024
	v_runs.push_back("378238");lumis.push_back(194.68+72.69); // - FULL  // ok
...
//----------------
	...
	c1->SaveAs("Vfd_Drop_Per_Layer_MultiScans.root");
...
//-- ACTIONS --//
  ...
  //!!
  bool UseFullScanModules = false; //Use full scan modules !! BECAREFUL, TAKES A LOT OF TIME TO LOOP OVER ALL MODULES
  //!! In most cases, you want it to false
//--------------------------------------------
...
//--- Can remove/add below scans if desired
	...
	for(int i=0; i<v_analysis.size(); i++)
	{
		for(int j=0; j<v_subdet.size(); j++)
		{
			...
			runs.push_back("193541");lumis.push_back(7.19);// OK
			...
			runs.push_back("254790");lumis.push_back(0.17+29.46);// OK
			runs.push_back("262254");lumis.push_back(4.23+29.46);// OK

			//2016 (5)
			// runs.push_back("271056");lumis.push_back(4.26+29.46); //Full, No B field
			runs.push_back("274969");lumis.push_back(7.58+29.46);// OK
			runs.push_back("276437");lumis.push_back(14.48+29.46);// OK
			// runs.push_back("276453");lumis.push_back(14.84+29.46); //Full, not completed (15v-195v)
			runs.push_back("278167");lumis.push_back(23.64+29.46);// OK
			// runs.push_back("279865");lumis.push_back(32.12+29.46); //FUll, not completed (225-350V)
			runs.push_back("280385");lumis.push_back(35.06+29.46);// OK
...
//
			...
			runs.push_back("298996");lumis.push_back(52.18+29.46);// OK
			runs.push_back("302131");lumis.push_back(65.84+29.46);// OK
			runs.push_back("303824");lumis.push_back(70.55+29.46); //-- FULL (~100fb-1)
			if(v_subdet[j] != "TOB") {runs.push_back("305862");lumis.push_back(91.65+29.46);} // OK //Low stat TOB -- (ALCARECO issue?)
			...
			runs.push_back("317182");lumis.push_back(113.01+29.46); // OK
            runs.push_back("317683");lumis.push_back(119.21+29.46);// OK
			runs.push_back("320674");lumis.push_back(127.08+29.46);// OK
            runs.push_back("323370");lumis.push_back(152.45+29.46); //FULL
            runs.push_back("324841");lumis.push_back(161.40+29.46); // OK
			runs.push_back("326776"); lumis.push_back(194);// OK
            // runs.push_back("326883");lumis.push_back(165.22+29.46); //Heavy Ions

			//Run 3 //Lumi  RUN II  = 165.22 fb-1: 
			//2021
			runs.push_back("346395");lumis.push_back(0.00000213+194.68); // OK
			//2022
			runs.push_back("353060");lumis.push_back(0.00000534+194.68); // //-- FULL // OK
			runs.push_back("359691");lumis.push_back(11.59+194.68); // OK
			runs.push_back("362696");lumis.push_back(40.35+194.68); // OK
			// runs.push_back("365843");lumis.push_back(41.42+194.68); //----------FULL // OK
			runs.push_back("368669"); lumis.push_back(58.16+194.68); // OK
			runs.push_back("373060"); lumis.push_back(72.69+194.68);// OK
			runs.push_back("375658"); lumis.push_back(73.86+194.68);//HI - NotOK
			// runs.push_back("378238");lumis.push_back(194.68+73.87); // - FULL  // OK
	  		runs.push_back("382655");  lumis.push_back(194.68+111.53); // OK
  			runs.push_back("385515");  lumis.push_back(194.68+172.75);// OK
			runs.push_back("386863"); lumis.push_back(194.68+194.48); // OK
			runs.push_back("388832");lumis.push_back(391); // OK noise scan
			
// /:Full scans only
			// runs.push_back("193928");lumis.push_back(7.41);// OK
			// runs.push_back("271056");lumis.push_back(4.26+29.46); //Full, No B field // OK
			// runs.push_back("295376");lumis.push_back(45.71+29.46); //-- FULL // OK
			// runs.push_back("303824");lumis.push_back(70.55+29.46); //-- FULL (~100fb-1) // OK
			// runs.push_back("314574");lumis.push_back(97.37+29.46); //-- FULL (-20°)// OK
			// runs.push_back("323370");lumis.push_back(152.45+29.46); //FULL // OK
			// runs.push_back("353060");lumis.push_back(195); // OK
			// runs.push_back("365843");lumis.push_back(194.68+41.42); // OK
			// runs.push_back("378238");lumis.push_back(194.68+72.69); // - FULL  // OK
			// runs.push_back("388832");lumis.push_back(391); // OK noise scan
			...
			{
                Plot_Mean_Vfd_Drop_Per_Layer(dirname, v_analysis[i], "",194.68+194.48);
			...
			if(draw_vfd_relative_evolution_plots) 
				{
					if (UseFullScanModules)
						{
							DrawDiffModules_FullScan(dirname, v_subdet[j], v_analysis[i], ref.Data(), runs.size(), runs, lumis, false, use_curvature, 0, draw_fit);
							DrawVfdModules_FullScan(dirname, v_subdet[j], v_analysis[i], ref.Data(), runs.size(), runs, lumis, false, use_curvature, 0, draw_fit);
						}
					else
						{
							DrawDiffModules_SmallScan(dirname, v_subdet[j], v_analysis[i], ref.Data(), runs.size(), runs, lumis, false, use_curvature, 0, draw_fit);
							DrawVfdModules_SmallScan(dirname, v_subdet[j], v_analysis[i], ref.Data(), runs.size(), runs, lumis, false, use_curvature, 0, draw_fit);
						}
				}
			if(draw_vfd_relative_evolution_superimposed_plots) 
				{
					if (UseFullScanModules)
						{
							Superimpose_DrawDiffModules_FullScan(dirname, v_subdet[j], v_analysis[i], ref.Data(), runs.size(), runs, lumis, false, use_curvature, draw_fit);
							Superimpose_DrawModules_FullScan(superimpose_simu, dirname, v_subdet[j], v_analysis[i], ref.Data(), runs.size(), runs, lumis, false, use_curvature, draw_fit);
						}
					else
						{
							Superimpose_DrawDiffModules_SmallScan(dirname, v_subdet[j], v_analysis[i], ref.Data(), runs.size(), runs, lumis, false, use_curvature, draw_fit);
							Superimpose_DrawModules_SmallScan(superimpose_simu, dirname, v_subdet[j], v_analysis[i], ref.Data(), runs.size(), runs, lumis, false, use_curvature, draw_fit);
						}


				} //RELATIVE VFD EVOL, SUPERIMPOSED FOR BOTH OBSERVABLES

			for(int k=3; k<8; k++) //Relative plots for all TEC layers
			{
				if(draw_vfd_relative_evolution_plots) 
					{
						DrawDiffModules_SmallScan(dirname, v_subdet[j], v_analysis[i], ref.Data(), runs.size(), runs, lumis, false, use_curvature, k, draw_fit);
						DrawVfdModules_SmallScan(dirname, v_subdet[j], v_analysis[i], ref.Data(), runs.size(), runs, lumis, false, use_curvature, k, draw_fit);
					} //RELATIVE VFD EVOLUTION, AVERAGED OVER MODULES
	...
	if(compute_mean_drop_multipleScans) {Plot_Mean_Vfd_Drop_Per_Layer_MultipleScans(dirname, "ClusterWidth");}//ClusterWidth is better,before type inversion, Cluster Charge is better after type inversion....
