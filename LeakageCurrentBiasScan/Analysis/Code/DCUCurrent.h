 ...
 std::string subdet="TIB", std::string bad_periods="", bool print=false, bool temperature=false)
{
  ...
  for(int i=0; i<nentries; i++)
  {
	...
	if (subdet=="TIB" && isubdet!=3) continue; 
	if (subdet=="TID" && isubdet!=4) continue; 
	if (subdet=="TOB" && isubdet!=5) continue; 
	if (subdet=="TEC" && isubdet!=6) continue;
	if (subdet=="ALL" && isubdet<=3 && isubdet>=6) continue; 	
